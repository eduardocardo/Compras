 ...
 */
public class ViewProductosRelacionados extends LinearLayout {
    ...
    private AsynctaskObtenerProductosRelacionados asynctaskObtenerProductosRelacionados;

    public ViewProductosRelacionados(Context context) {
        super(context);
        init();
    }

    public ViewProductosRelacionados(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ViewProductosRelacionados(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        titulo.setText(R.string.productos_relacionados);

        TextViewTypeface cargando = (TextViewTypeface) view.findViewById(R.id.text_cargando);
        cargando.setText(R.string.cargando_productos_relacionados);
        ...
        if (!isInEditMode()) {
            asynctaskObtenerProductosRelacionados = new AsynctaskObtenerProductosRelacionados();
        }
    ...
    public void cargarDatos() {
        if (asynctaskObtenerProductosRelacionados != null) {
            asynctaskObtenerProductosRelacionados.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        }
    }
    ...
    protected void onDetachedFromWindow() {
        if (asynctaskObtenerProductosRelacionados != null) {
            asynctaskObtenerProductosRelacionados.cancelar();
        }

        super.onDetachedFromWindow();
    }

    private class AsynctaskObtenerProductosRelacionados extends AsyncTask<Void, Void, Articulo[]> {
        ...
        protected Articulo[] doInBackground(Void... voids) {
            //se generan articulos con datos obtenidos de forma aleatoria
...
//                    .post(formBody)
            ...
            try {
                ...
                switch (response.code()) {
                    case 200: {
                        ...
                        Utilidades.mostrarLog("RELACIONADOS", "Url: " + url);
                        Utilidades.mostrarLog("RELACIONADOS", "(" + ((System.currentTimeMillis() - initMillis) / 1000.0) + " segs) Respuesta: " + respuesta);
        ...
        protected void onPostExecute(final Articulo[] articulos) {
            ...
            if (articulos == null && Config.accessToken != null) {
                AsynctaskObtenerProductosRelacionados asynctaskObtenerProductosRelacionados = new AsynctaskObtenerProductosRelacionados();
                asynctaskObtenerProductosRelacionados.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
